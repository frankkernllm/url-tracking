...
exports.handler = async (event, context) => {
  ...
  try {
    ...
    const conversionData = await getConversionsFromIndexesFast(redis, startDate, endDate, Math.min(limit, 200), 8000);
    ...
    if (conversionData.conversions.length > 0) {
      ...
      return {
        ...
        body: JSON.stringify({
          ...
          conversions: conversionData.conversions.slice(0, 50), // Limit for dashboard
    ...
    const journeyData = await getJourneyDataLimited(redis, startDate, endDate, Math.min(limit, 100), maxProcessingTime - (Date.now() - startTime));
        ...
        if (indexData?.result) {
          ...
          if (dateIndex.conversions && Array.isArray(dateIndex.conversions)) {
            ...
            const dateConversions = dateIndex.conversions
              ...
              .slice(0, Math.min(50, limit - conversions.length)); // Limit per day
...
async function getJourneyDataLimited(redis, startDate, endDate, limit, maxTime) {
  ...
  let cursor = '0';
  let iterations = 0;
  const maxIterations = 8; // Strict limit
  ...
  try {
    do {
      ...
      const limitedKeys = keys.slice(0, Math.min(25, limit - conversions.length));
