...
// 🔧 UPDATED: Fixed IP extraction AND added attribution parameter storage
  ...
  try {
    ...
    if (extractedData.SSID) {
      ...
      try {
        ...
        if (sessionResult) {
          attributionResult = JSON.parse(sessionResult);
      ...
      for (const priority of ipPriorities) {
        if (!attributionResult && priority.ip) {
          ...
          try {
            ...
            if (ipResult) {
              attributionResult = JSON.parse(ipResult);
              attributionAttempts.attribution_found = true;
    ...
    if (!attributionResult && extractedData.dsig) {
      ...
      try {
        ...
        if (deviceResult) {
          attributionResult = JSON.parse(deviceResult);
          attributionAttempts.attribution_found = true;
    ...
    try {
      await redis.set(conversionKey, JSON.stringify(enhancedTrackingData));
