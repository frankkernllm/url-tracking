// DEEP.JS v6.0: Complete Implementation with Full Geographic Correlation
// Processes conversions without "deep6" marker using robust 8-tier + 24-hour geo correlation
// Reprocesses deep4/deep5 conversions with enhanced resilient timeout handling
...
// ================================================================
// 2. DEEP6 FILTERING AND MARKING FUNCTIONS
// ================================================================

async function filterNonDeep6Conversions(unattributedConversions) {
    const unprocessedConversions = [];
    let alreadyProcessedDeep6Count = 0;
    const maxProcessingTime = 30000;
    const startTime = Date.now();
    
    console.log(`🔍 Checking ${unattributedConversions.length} conversions for deep6 processing status...`);
    ...
    for (const conversion of unattributedConversions) {
        ...
        // ONLY check for deep6 marker (ignore deep2, deep3, deep4, deep5, etc.)
        const keyDeep6 = `deep6:${conversion.email}:${conversion.timestamp}`;
        
        try {
            const processedData = await redisRequest('get', keyDeep6);
            
            if (processedData) {
                alreadyProcessedDeep6Count++;
                console.log(`   ⏭️ Skipping [PRIVACY PROTECTED] - already processed with deep6 system`);
            } else {
                // No deep6 marker found - process this conversion
                // (This includes conversions with deep2, deep3, deep4, deep5 markers that need reprocessing)
                unprocessedConversions.push(conversion);
            }
        } catch (error) {
            console.log(`   ⚠️ Failed to check deep6 status: ${error.message}`);
            unprocessedConversions.push(conversion);
        }
    }
    
    console.log(`📊 Filtered out ${alreadyProcessedDeep6Count} already processed with deep6 system`);
    console.log(`🔄 Will reprocess ${unprocessedConversions.length} conversions (includes deep2/deep3/deep4/deep5 for reprocessing)`);
...
async function markConversionAsDeep6(conversion, attributionMethod, retryCount = 0) {
    const maxRetries = 3;
    
    try {
        const deep6Key = `deep6:${conversion.email}:${conversion.timestamp}`;
        const deep6Data = {
            email: conversion.email,
            timestamp: conversion.timestamp,
            processed_at: new Date().toISOString(),
            system: 'deep6_8tier_resilient_geocorr',
            version: '6.0',
            attribution_method: attributionMethod,
            processing_type: 'deep_dive_analysis_resilient',
            retry_count: retryCount
        };
        
        await redisRequest('setex', deep6Key, 2592000, JSON.stringify(deep6Data));
        console.log(`   ✅ Marked conversion as deep6 processed (attempt ${retryCount + 1})`);
        
    } catch (error) {
        console.log(`   ⚠️ Could not mark conversion as deep6: ${error.message}`);
        
        if (retryCount < maxRetries) {
            console.log(`   🔄 Retrying mark operation (${retryCount + 1}/${maxRetries})...`);
            await new Promise(resolve => setTimeout(resolve, 1000 * (retryCount + 1)));
            return markConversionAsDeep6(conversion, attributionMethod, retryCount + 1);
        } else {
...
function prepareConversionDataForDeep6(conversion) {
    ...
    return {
        ...
        processing_type: 'deep_dive_24h_resilient'
    };
}
...
async function analyzeConversionForAttributionDeep6(conversion, pageviews, conversionData, geoDataCache, cacheStats) {
...
async function updateConversionAttributionDeep6(conversion, improvementResults) {
    try {
        ...
        if (conversionKey) {
            ...
            const updatedConversion = {
                ...
                source: improvementResults.match.pageview?.source || improvementResults.match.source || 'deep6_enhanced',
                ...
                attribution_improvement: {
                    ...
                    system_version: '6.0',
                    processing_path: improvementResults.analysis?.processing_path || 'deep6_resilient',
                    window_size: '24_hours',
...
exports.handler = async (event, context) => {
    ...
    try {
        console.log('🔍 Starting DEEP DIVE ATTRIBUTION SYSTEM v6.0 - Resilient Timeout Handling');
        console.log('⚡ Enhanced: Complete 8-tier priority system + 24-hour geo correlation + resilient timeout handling');
        console.log('🔄 Reprocesses deep4/deep5 conversions with improved resilience');
        ...
        // Step 3: Filter out already processed deep6 conversions
        const unprocessedDeep6Conversions = await filterNonDeep6Conversions(unattributedConversions);
        
        if (unprocessedDeep6Conversions.length === 0) {
            const message = unattributedConversions.length > 0 
                ? `🎯 All ${unattributedConversions.length} unattributed conversions have been processed with deep6 system`
            ...
            return {
                ...
                body: JSON.stringify({
                    ...
                    results: { 
                        ...
                        unprocessed_deep6: 0,
                        status: 'ALL_DEEP6_PROCESSED'
                    }
                })
            };
        }

        console.log(`🎯 Found ${unprocessedDeep6Conversions.length} not yet processed with deep6 system (includes deep4/deep5 for reprocessing)`);
        
        // Step 4: Process first conversion (most recent unattributed)
        const conversionToProcess = unprocessedDeep6Conversions[0];
        console.log(`🔍 Processing conversion: [PRIVACY PROTECTED] from ${conversionToProcess.timestamp} (most recent unattributed)`);
        console.log(`   🔍 Using enhanced 8-tier system with 24-hour window (deep6 version)`);
        
        // Step 5: Prepare conversion data
        const conversionData = prepareConversionDataForDeep6(conversionToProcess);
        ...
        const improvementResults = await analyzeConversionForAttributionDeep6(
            conversionToProcess, 
        ...
        if (improvementResults.matchFound && improvementResults.shouldUpdate) {
            ...
            try {
                updateResult = await updateConversionAttributionDeep6(conversionToProcess, improvementResults);
        ...
        // Step 8: Mark as processed with deep6
        await markConversionAsDeep6(conversionToProcess, improvementResults.attributionMethod || 'none');
        
        // Step 9: Generate response
        const remainingUnprocessedDeep6 = unprocessedDeep6Conversions.length - 1;
        const processingTime = Date.now() - startTime;
        
        let summaryMessage;
        if (improvementResults.matchFound) {
            summaryMessage = `✅ Found attribution using ${improvementResults.attributionMethod} (Priority ${improvementResults.priorityLevel})! ${remainingUnprocessedDeep6} conversions remaining.`;
        } else {
            summaryMessage = `❌ No attribution found even with complete 8-tier system + 24-hour geo correlation. ${remainingUnprocessedDeep6} conversions remaining.`;
        }
        
        console.log(`\n🏁 DEEP DIVE v6.0 COMPLETE:`);
        ...
        return {
            ...
            body: JSON.stringify({
                ...
                results: {
                    total_conversions: allConversions.length,
                    unattributed: unattributedConversions.length,
                    unprocessed_deep6: unprocessedDeep6Conversions.length,
                    processed_this_run: 1,
                    remaining_unprocessed_deep6: remainingUnprocessedDeep6,
                    ...
                    status: remainingUnprocessedDeep6 > 0 ? 'MORE_TO_PROCESS' : 'ALL_DEEP6_PROCESSED',
                    next_action: remainingUnprocessedDeep6 > 0 ? 'Press button again to process next conversion' : 'All unattributed conversions have been processed with deep6 system',
                    timeout_protection: 'enabled',
                    retry_logic: 'enabled',
                    geographic_correlation: 'enabled_24h_resilient',
        ...
        console.error('❌ Deep6 system error:', error);
